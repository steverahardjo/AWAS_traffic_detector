...
class kafkaProducer_Violation:
    """
    Kafka Producer class for producing violation data to Kafka.
    
    Args:
        csv_path (str): Path to the CSV file containing violation data
        kafka_server (str): Kafka server address or env var name
        producer_id (str): Unique identifier for this producer
        topic (str): Kafka topic to produce to
    """
    def __init__(self, csv_path: str, kafka_server: str, producer_id: str, topic: str, batch_interval: float = 5):
        ...
        try:
            self.df = pd.read_csv(
                csv_path  # only timestamp_start parsed
            )

            self.df.columns = self.df.columns.str.strip()
            
            if 'timestamp_start' in self.df.columns:
            ...
            self.df = self.df.sort_values(by="timestamp_start")
